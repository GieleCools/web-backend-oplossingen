...
<head>
	<meta charset="UTF-8">
	<title><?= ($idIsset)? $artikels[$individueelArtikelID]['titel'] : 'Oplossing opdracht-get-Overzicht' ?></title>
...
<body>
		...
		<h1>
			<?= ($idIsset)? 'Individueel artikel' : 'De krant van vandaag' ?>
		</h1>
		...
		<?php foreach ($artikels as $key => $value): ?>
			...
			{
				echo 'individueelArtikel';
			}
			elseif($idIsset && $key!=$individueelArtikelID)
			{
				echo 'verbergArtikel';
			}
			else
			{
				echo 'meerdereArtikels';
			}
			?>">
				<h2><?= $value['titel']; ?></h2>
				<h5><?= $value['datum']; ?></h5>
